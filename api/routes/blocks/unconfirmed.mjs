...
router.get('/summary', async (req, res) => {
  ...
  try {
    ...
    const unconfirmed_blocks_by_type_and_subtype = await db('blocks')
      ...
      .then((rows) => {
        return rows.reduce((acc, row) => {
          const typeKey = Object.keys(constants.blockType).find(
            (key) => constants.blockType[key] === row.type
          )
          const subtypeKey = row.subtype
            ? Object.keys(constants.blockSubType).find(
                (key) => constants.blockSubType[key] === row.subtype
              )
            : 'none'
          const combinedKey = `${typeKey.toLowerCase()}_${subtypeKey.toLowerCase()}`
          acc[combinedKey] = Number(row.count)
          return acc
        }, {})
      })

    const unconfirmed_blocks_by_bucket = await db
      .raw(
        `
      SELECT 
        CASE 
    ...
    `
      ...
      .then((resp) => {
        const { rows } = resp
        const buckets = Array(62).fill(0) // Initialize buckets
        rows.forEach((row) => {
          buckets[row.bucket_index] = row.count
        })
        return buckets.reduce((acc, count, index) => {
          acc[`bucket_${index}`] = Number(count)
          return acc
        }, {})
      })
    ...
    res.status(200).send({
      ...
      unconfirmed_blocks_by_type_and_subtype
    })
  } catch (error) {
    logger(error)
